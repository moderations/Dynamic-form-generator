<template>
    <el-form :model="configData" :rules="rules" ref="form" size="mini" label-width="100px" label-position="left" class="form-element-config form-element-config--select">
        ...
        <el-form-item label="数据类型" prop="valueType">
            <el-select v-model="configData.valueType" disabled class="form-el-width-180">
        ...
        <el-form-item label="默认值" prop="defaultValue">
            <el-select v-model="configData.defaultValue" :disabled="!isDefaultValueSelectable" multiple class="form-el-width-180">
                <el-option v-for="option in defaultValueOptions" :key="option" :label="option" :value="option"></el-option>
            </el-select>
            <span class="c-info" style="margin-left: 10px;">选项列表有数据才可选择</span>
        </el-form-item>
        ...
        <el-form-item label="选项列表" prop="options">
            ...
            <template v-if="!isAddOptionDisabled">
                <div v-for="(option, index) in configData.options" :key="index" class="form-element-config--select-option" flex style="flex-wrap: wrap">
                    ...
                    <i @click="configData.options.splice(index, 1)" class="el-icon-error c-danger"></i>
                </div>
            </template>
        </el-form-item>
        <el-form-item>
...
export default {
    ...
    data() {
        ...
        const optionsValidator = (rule, value, callback) => {
            if (!value || !value.length) {
                return callback(new Error('选项列表不能为空'));
            }
            if(value.some(o => !o.label || o['value'] === '' || o['value'] === undefined)) {
                return callback(new Error(`请完善选项列表`));
            }
            if(this.checkDuplicated(value, 'value')) {
                return callback(new Error(`value值不能重复`))
            }
            callback();
        };
        return {
            rules: {
                ...
                options: [
                    {required: true, validator: optionsValidator, trigger: 'submit'}
                ],
            },
            visible: true,
            configData: {},
    ...
    computed: {
        ...
        isDefaultValueSelectable() {
            const {valueType, configData} = this;
            const {options} = configData;

            return valueType && options.some(option => option.value !== undefined);
        },
        ...
        defaultValueOptions() {
            ...
            return !valueType ? [] : [...new Set(options.filter(option => option.value !== undefined).map(option => option.value))]
        }
    },
    ...
    watch: {
        config: {
            handler(newVal, oldVal) {
                ...
                this.valueType = options && options.length ? typeof options[0].value : undefined;
                this.isButton = !!attrs && attrs.type == 'button';
    ...
    methods: {
        onValueTypeChange() {
            this.$set(this.configData, 'defaultValue', []);
            this.$set(this.configData, 'options', cloneDeep(this.configData.options.map(o => ({...o, value: undefined}))));
        },
