<template>
    <el-form :model="configData" :rules="rules" ref="form" size="mini" label-width="100px" label-position="left" class="form-element-config form-element-config--select">
        ...
        <el-form-item label="原生type" prop="attrs.type">
            <!-- inputnumber 类型为数值 不允许更改 -->
            <el-select v-model="configData.attrs.type" class="form-el-width-180">
        ...
        <el-form-item label="默认值" prop="defaultValue">
            <el-input v-model="configData.defaultValue" class="form-el-width-180"></el-input>
        </el-form-item>
        ...
        <!-- <el-form-item label="最小输入长度" prop="attrs.minlength">
            <el-input-number v-model="configData.attrs.minlength" class="form-el-width-180"></el-input-number>
        </el-form-item> -->
        <el-form-item label="最大输入长度" prop="attrs.maxlength">
            <el-input-number v-model="configData.attrs.maxlength" class="form-el-width-180"></el-input-number>
        </el-form-item>
        <el-form-item label="显示字数统计" prop="attrs.showWordLimit">
            <el-switch v-model="configData.attrs.showWordLimit"></el-switch>
        </el-form-item>
...
export default {
    ...
    data() {
        ...
        const lengthValidator = (rule, value, callback) => {
            if (value && value<0) {
                return callback(new Error('长度填写有误'));
            }
            callback();
        };
        // const minLengthValidator = (rule, value, callback) => {
        //     if (value && this.configData.attrs.maxlength && value > this.configData.attrs.maxlength) {
        //         return callback(new Error('最小输入长度不能大于最大输入长度'));
        //     }
        //     callback();
        // };
        // const maxLengthValidator = (rule, value, callback) => {
        //     if (value && this.configData.attrs.minlength && value < this.configData.attrs.minlength) {
        //         return callback(new Error('最大输入长度不能小于最小输入长度'));
        //     }
        //     callback();
        // };
        return {
            rules: {
                ...
                // 'attrs.minlength': [
                //     {validator: lengthValidator, trigger: 'submit'},
                //     {validator: minLengthValidator, trigger: 'submit'}
                // ],
                'attrs.maxlength': [
                    {validator: lengthValidator, trigger: 'submit'},
                    // {validator: maxLengthValidator, trigger: 'submit'}
                ],
            },
            visible: true,
            configData: {},
        ...
        config: {
            handler(newVal, oldVal) {
                const {
                    ...
                    attrs: {
                        placeholder = '',
                        'show-word-limit': showWordLimit = false,
                        clearable = false,
                        maxlength,
                        // minlength,
                    } = {}, 
        ...
        onSave() {
            return new Promise((resolve, reject) => {
                this.$refs.form.validate(async valid => {
                    ...
                    const {attrs: {showWordLimit, ...otherAttrs} = {}, ...other} = configData;
                    ...
                    const _configData = cloneDeep({
                        ...
                        attrs: {
                            ...otherAttrs,
                            // required: otherAttrs.minlength ? true : false,
