<template>
    <el-form :model="configData" :rules="rules" ref="form" size="mini" label-width="100px" label-position="left" class="form-element-config form-element-config--select">
        ...
        <el-form-item label="选项列表" prop="radioGroupOptions">
            <el-button :disabled="isAddOptionDisabled" @click="onAddOption" type="primary">新增</el-button>
            <template v-if="!isAddOptionDisabled">
                <div v-for="(option, index) in configData.radioGroupOptions" :key="index" class="form-element-config--select-option" flex="cross:center" style="flex-wrap: wrap">
                    <el-input 
                        v-model="option.label" 
                        placeholder="请输入选项显示文本" 
                        clearable 
                    ...
                    <i @click="configData.radioGroupOptions.splice(index, 1)" class="el-icon-error c-danger"></i>
                </div>
            </template>
        </el-form-item>
        <el-form-item>
...
export default {
        ...
        config: {
            handler(newVal, oldVal) {
                ...
                this.configData = {
                    ...
                    radioGroupOptions: cloneDeep(radioGroupOptions),
                };
    ...
    methods: {
        onValueTypeChange() {
            ...
            this.$set(this.configData, 'radioGroupOptions', cloneDeep(this.configData.radioGroupOptions.map(o => ({...o, value: undefined}))));
        },
        ...
        onAddOption() {
            this.configData.radioGroupOptions.push({
                label: '',
                value: undefined
            });
        },
